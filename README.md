...
1. Mở **UXP Developer Tool** → **Add Plugin** → chọn `plugin/manifest.json` (chỉ để UDT nhận diện, chưa phải bước cài cuối).
...
2. Bấm **Add Plugin**, chọn file `plugin/manifest.json`.
...
**Gói riêng để chia sẻ ra ngoài** (chỉ 4 file, không thư mục con — tránh người mới nhầm lẫn giữa nhiều file): xem thư mục `Mic Check Plugin - Share/` (nằm ngay trong repo, cùng cấp với README.md này), gồm `MicCheck_v<version>.ccx` + `File_Mau_Docx_Mic_Check.docx` + `Chuyen_Doi_File_Mic_Check.exe` + `HUONG_DAN_SU_DUNG.txt`.
